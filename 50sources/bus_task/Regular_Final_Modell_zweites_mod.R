...
#Daten laden 
setwd("C:/Users/Max Weber/Desktop/AI-Cup/ai-cup-2022-ace/30data/db_regio_bus/modified")

regular <- read.csv("regular_travel_mod_wdw_feat.csv")
...
setwd("C:/Users/Max Weber/Desktop/AI-Cup/ai-cup-2022-ace/30data/db_regio_bus/Wetterdaten/")
wetter <- read.csv2("Dat_Wetter_mod.csv")

wetter$date <- as.Date(wetter$date)

wetter <- subset(wetter, date < "2019-12-17")
...
for(f in 1:13){
  ...
  for(p in 13:62){
    ...
    #Training
      ...
      sub_dat_ohne_na <- sub_dat[1:(8065+167),]
      sub_dat_ohne_na <- sub_dat_ohne_na[complete.cases(sub_dat_ohne_na$class),]
    }
    ...
    if(f!=13){
      ...
      sub_dat_mit_na <- sub_dat[(8066+167):(8065+335),]
    }
  ...
  mod0 <- xgboost(data =xgb_train , max_depth = 20,  eta = 0.05, nrounds = 350  ,
  ...
  mod1 <- xgboost(data =xgb_train , max_depth = 20,  eta = 0.05, nrounds = 350 ,
  ...
  watchlist <- list(train = xgb_train, eval = xgb_test)
    ...
    mod2 <- xgboost(data =xgb_train , max_depth = 20,  eta = 0.05, nrounds = 350 ,
  ...
  # Define the parameters for binary classification
    ...
    mod3 <- xgboost(data =xgb_train , max_depth = 20, eta = 0.05, nrounds = 350 ,
